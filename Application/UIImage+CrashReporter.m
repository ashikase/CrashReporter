...
+ (instancetype)imageWithColor:(UIColor *)color {
    ...
    // Create 1x1 image.
    const CGRect rect = CGRectMake(0.0, 0.0, 1.0, 1.0);
...
+ (instancetype)imageWithText:(NSString *)text font:(UIFont *)font color:(UIColor *)color imageSize:(CGSize)imageSize {
    ...
    const CGSize textSize = [text sizeWithFont:font constrainedToSize:imageSize];
    UIGraphicsBeginImageContextWithOptions(imageSize, NO, 0.0);
    [color setFill];
    const CGPoint point = CGPointMake(0.5 * (imageSize.width - textSize.width), 0.5 * (imageSize.height - textSize.height));
